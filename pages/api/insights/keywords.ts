...
export default async function handler(
  ...
  try {
    ...
    // キーワード出現頻度を取得
    const { data, error } = await supabaseClient.rpc('get_keyword_frequency', {
      p_site_id: site_id,
    ...
    if (error) {
      console.error('[Insights API] Error fetching keyword frequency:', error);
      return res.status(500).json({ 
        message: 'Failed to fetch keyword frequency',
        error: error.message 
      });
    }
